"""
PaperViz PDF Service — 配置模块
所有设置从环境变量加载。

本服务隔离 PyMuPDF (AGPL-3.0) 代码，提供 PDF 文本提取功能。
...
for _base in _search_dirs:
    ...
    if _env_file.is_file():
        ...
        _loaded = True
        print(f"[PDF Service] 已加载环境文件: {_env_file} (APP_ENV={APP_ENV})")
        break
...
if not _loaded:
    load_dotenv()
    print("[PDF Service] 警告: 未找到任何 .env 文件，使用系统环境变量", file=sys.stderr)
...
# ── PDF 提取限制 ─────────────────────────────────────────
PDF_EXTRACT_MAX_FILE_SIZE = int(os.getenv("PDF_EXTRACT_MAX_FILE_SIZE", str(100 * 1024 * 1024)))
PDF_EXTRACT_MAX_PAGES = int(os.getenv("PDF_EXTRACT_MAX_PAGES", "500"))
PDF_EXTRACT_TASK_TIMEOUT = int(os.getenv("PDF_EXTRACT_TASK_TIMEOUT", "300"))
